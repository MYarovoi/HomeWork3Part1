...
for (index, value) in cart.enumerated() {
    ...
    print("-------\(index + 1)------- \nНазва товару: \(cart[index].0), Ціна: \(cart[index].1) \nСокет: \(cart[index].3), Процессор: \(cart[index].4) \n\n")
}
...
for (index, value) in cart.enumerated() {
    ...
    print("-------\(index + 1)------- \nНазва товару: \(cart[index].0) \nПроцессор: \(cart[index].4) \n\n")
}
...
for (index, value) in cart.enumerated() {
    ...
    if cart[index].1 <= 5000 && cart[index].3 == "s1151" {
...
for (index, value) in cart.enumerated() {
    ...
    var processorName = cart[index].4
    
    switch processorName {
        
    case "Intel":
        print("-Ви обрали Intel- \n-------\(index + 1)------- \nНазва товару: \(cart[index].0), \nЦіна: \(cart[index].1) \n\n")
        
    case "AMD":
        print("-Ви обрали AMD- \n-------\(index + 1)------- \nНазва товару: \(cart[index].0), \nЦіна: \(cart[index].1) \n\n")
        
    default:
        print("-Ви обрали іншого виробника- \n-------\(index + 1)------- \nНазва товару: \(cart[index].0), \nЦіна: \(cart[index].1) \n\n")
    }
}
...
func printIntelCartsOnly () {
    
    print("-----Intel-----")
    
    for (index, value) in cart.enumerated() {
        
        if cart[index].4 == "Intel" {
            
            print("\(index + 1) Назва товару: \(cart[index].0)")
        }
    }
}
...
func maxPriceFilter (maxPrice: Double) {
    
    print("\n\n-----Товари з ціною менше \(maxPrice)-----")
    
    for (index, value) in cart.enumerated() {
        
        if maxPrice >= cart[index].1 {
            
            var newItem = "Назва товару: \(cart[index].0), Ціна: \(cart[index].1) ₴"
            filtredArray.append(newItem)
        }
    }
    
    // Используем дополнительный цикл для того, чтобы вывести нумерацию, поскольку если использовать index из cart он будет следовать порядковым номерам элементов массива.
    for (index, value) in filtredArray.enumerated() {
        
        print("\(index + 1) \(value)")
    }
}

maxPriceFilter(maxPrice: 3000)
...
func maxPriceSelectedProcessor (processor: String) {
    
    var mostExpensiveCart: ProductInfo?
    var highestPriceIndex = 0
    var highestPrice: Double = 0
    
    for (index, value) in cart.enumerated() {
        
        if cart[index].1 > highestPrice && cart[index].4 == processor {
            
            highestPriceIndex = index
            highestPrice = cart[index].1
        }
    }
    
    mostExpensiveCart = cart[highestPriceIndex]
    
    print("\n\n---Найдорожчий товар за процесором \(processor)---")
    
    if let cartName = mostExpensiveCart?.0, let price = mostExpensiveCart?.1 {
        print("Назва товару: \(cartName), Ціна: \(price) ₴")
    }
}

maxPriceSelectedProcessor(processor: "AMD")
...
let processor: ProcessorType = .amd
